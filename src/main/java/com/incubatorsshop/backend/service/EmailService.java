...
public class EmailService {
	...
	public void sendOrderNotificationToAdmin(Order order) {
		try {
			...
			if (adminOpt.isEmpty() || adminOpt.get().getEmail() == null) {
				System.err.println("No Admin user with a valid email found in the database. Email skipped.");
				return;
			}
			...
			// Fixed the repetitive "Product Id" labels here
			...
			System.out.println("Order Notification Email successfully sent to Admin: " + adminEmail);

		} catch (MessagingException e) {
			System.err.println("Failed to send order email to admin: " + e.getMessage());
		}
	}
	...
	public void sendOrderConfirmationToCustomer(Order order) {
		...
		// Check if the customer actually provided an email (since it's optional in your
		// UI)
		if (order.getUser() == null || order.getUser().getEmail() == null
				|| order.getUser().getEmail().trim().isEmpty()) {
			System.out.println("Customer did not provide an email address. Customer email skipped.");
			return;
		}
		...
		try {
			...
			System.out.println("Order Confirmation Email successfully sent to Customer: " + order.getUser().getEmail());

		} catch (MessagingException e) {
			System.err.println("Failed to send order email to customer: " + e.getMessage());
		}
	}
	...
	public void sendReviewThankYouEmail(User user, Product product) {
		...
		try {
			...
			System.err.println("Review Email Failed: " + e.getMessage());
		}
	}
